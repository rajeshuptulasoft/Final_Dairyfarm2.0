...
import { Plus, Save, X, Search, Edit2, Trash2, Truck, Building2 } from 'lucide-react';
...
export default function Vendors() {
  ...
  const [form, setForm] = useState(getInitialVendorForm);
  const [editForm, setEditForm] = useState(getInitialVendorForm);
  ...
  const load = async () => {
    try {
      const res = await vendorsAPI.list({ limit: 200 });
      const list = parseVendorList(res.data);
      if (list) setRecords(list);
    } catch (err) {
      toast.error(err.response?.data?.error || 'Error loading vendors');
    }
  };
  ...
  const closeAdd = () => {
    setShowForm(false);
    setForm(getInitialVendorForm());
  };

  const closeEdit = () => {
    setShowEdit(false);
    setEditId(null);
    setEditForm(getInitialVendorForm());
  };

  const vendorErrorMessage = (err) =>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(res.data?.message || 'Vendor added');
      closeAdd();
      await load();
    } catch (err) {
      toast.error(vendorErrorMessage(err));
    }
    setSaving(false);
  };
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const res = await vendorsAPI.update(editId, payload);
      setRecords((prev) =>
      ...
      toast.error(err.response?.data?.error || 'Error updating vendor');
    }
    setSaving(false);
  };
  ...
  const openEdit = (r) => {
    setEditForm(vendorFromRecord(r));
    setEditId(r.id);
    setShowEdit(true);
  };
        ...
        <div className="modal-container">
          <div className="modal-content">
            ...
            <form onSubmit={onSubmit}>
              <VendorFormFields form={formState} setForm={setFormState} />
      ...
      <AdminTablePanel noPadding>
          <table className="w-full admin-data-table">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center py-16" style={{ color: 'var(--text-soft)' }}>
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="font-semibold text-base" style={{ color: 'var(--text)' }}>
                        {search ? 'No matches' : 'No vendors yet'}
                      </p>
                      <p className="text-sm">Add your first vendor using the button above</p>
                    </div>
                  </td>
                </tr>
              ) : (
