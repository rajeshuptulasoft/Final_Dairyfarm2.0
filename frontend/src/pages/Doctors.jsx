...
import { Plus, Save, X, Search, Edit2, Trash2, HeartPulse, User, Star } from 'lucide-react';
...
export default function Doctors() {
  ...
  const [form, setForm] = useState(getInitialDoctorForm);
  const [editForm, setEditForm] = useState(getInitialDoctorForm);
  ...
  const load = async () => {
    try {
      const res = await doctorsAPI.list({ limit: 200 });
      const list = parseDoctorList(res.data);
      if (list) setRecords(list);
    } catch (err) {
      toast.error(err.response?.data?.error || 'Error loading doctors');
    }
  };
  ...
  const closeAdd = () => {
    setShowForm(false);
    setForm(getInitialDoctorForm());
  };

  const closeEdit = () => {
    setShowEdit(false);
    setEditId(null);
    setEditForm(getInitialDoctorForm());
  };

  const doctorErrorMessage = (err) =>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(doctorErrorMessage(err));
    }
    setSaving(false);
  };
  ...
  const handleUpdate = async (e) => {
    ...
    setSaving(true);
    try {
      const payload = buildDoctorPayload(editForm);
      const res = await doctorsAPI.update(editId, payload);
      setRecords((prev) =>
      ...
      toast.error(err.response?.data?.error || 'Error updating doctor');
    }
    setSaving(false);
  };
  ...
  const openEdit = (r) => {
    setEditForm(doctorFromRecord(r));
    setEditId(r.id);
    setShowEdit(true);
  };
        ...
        <div className="modal-container">
          <div className="modal-content" style={{ maxWidth: '720px' }}>
            ...
            <form onSubmit={onSubmit}>
              <DoctorFormFields form={formState} setForm={setFormState} />
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          <div
            ...
            style={{ background: 'rgba(20,184,166,.12)', color: '#14b8a6' }}
          >
            <HeartPulse size={22} />
          </div>
          <div>
      ...
      <AdminTablePanel noPadding>
          <table className="w-full admin-data-table">
            <thead>
              ...
              >
                <th className="text-left px-3 py-3">Doctor</th>
                <th className="text-left px-3 py-3">Email</th>
                <th className="text-left px-3 py-3">Phone</th>
                <th className="text-left px-3 py-3">Cost/Visit</th>
                <th className="text-left px-3 py-3">Cost/Hour</th>
                <th className="text-left px-3 py-3">Cost/Online</th>
                  ...
                  <td colSpan={8} className="text-center py-16" style={{ color: 'var(--text-soft)' }}>
                    <div className="flex flex-col items-center gap-3">
                      <div
                        ...
                        style={{ background: 'rgba(20,184,166,.08)', color: '#14b8a6' }}
                      >
                        <HeartPulse size={28} />
                      </div>
                      <p className="font-semibold text-base" style={{ color: 'var(--text)' }}>
                        {search ? 'No matches' : 'No doctors yet'}
                      </p>
                    ...
                    <td className="px-3 py-3 text-sm">
                      {r.rating != null && r.rating !== '' ? (
                        <span
                          ...
                          style={{ background: 'rgba(251,191,36,.1)', color: '#d97706' }}
                        >
                          <Star size={12} />
                          {r.rating}
                        </span>
